...
#ifdef AT91_ETH_USE_POLL
	rtems_id						poll_task_id;
	rtems_name						poll_task_name;
#endif
...
static int at91sam9g25_emac_ioctl (struct ifnet *ifp,
                                  ...
                                  caddr_t data);
#ifdef AT91_ETH_USE_POLL
rtems_task at91_eth_poll_task(rtems_task_argument unused);
#endif
...
void at91sam9g25_emac_init(void *arg)
{
    ...
    if (sc->txDaemonTid == 0) 
    {
...
#ifdef AT91_ETH_USE_POLL
		{
			...
			sc->poll_task_name = rtems_build_name('A', 'T', 'E', 'P');
			status = rtems_task_create(sc->poll_task_name, 1, RTEMS_MINIMUM_STACK_SIZE * 4, RTEMS_DEFAULT_MODES, RTEMS_DEFAULT_ATTRIBUTES, &sc->poll_task_id);
			if (status != RTEMS_SUCCESSFUL)
				rtems_panic("can't allocate at91 eth poll task.\n\r");
			status = rtems_task_start( sc->poll_task_id, at91_eth_poll_task, 0);
			if (status != RTEMS_SUCCESSFUL)
				rtems_panic("can't start at91 eht poll task.\n\r");
		}
#endif
    } /* if txDaemonTid */
...
void at91sam9g25_emac_txDaemon (void *arg)
{
    ...
    for (;;)
    {
        ...
        if (sc->txLedStatus)
        {
            PIO_Clear(&at91sam9g25_ethernet_led_pins[0]);
            sc->txLedStatus = 0;
        }
        else
        {
            PIO_Set(&at91sam9g25_ethernet_led_pins[0]);
            sc->txLedStatus = 1;
        }
...
void at91sam9g25_emac_rxDaemon(void *arg)
{
    ...
    for (;;) {
        ...
        if (sc->rxLedStatus)
        {
            PIO_Clear(&at91sam9g25_ethernet_led_pins[1]);
            sc->rxLedStatus = 0;
        }
        else
        {
            PIO_Set(&at91sam9g25_ethernet_led_pins[1]);
            sc->rxLedStatus = 1;
        }
#endif
...
static void at91sam9g25_emac_isr (rtems_irq_hdl_param unused)
{
	...
	rsr = EMAC->EMAC_RSR;
	tsr = EMAC->EMAC_TSR;
	intmask = EMAC->EMAC_IMR;
	status32 &= ~(intmask | 0xFFC300);

	DB(printk("."));
	softc.Interrupts++;
    if ((status32 & EMAC_ISR_RCOMP) || (rsr & EMAC_RSR_REC))   {
		/* disable the RX interrupts */
		EMAC->EMAC_RSR = EMAC_RSR_REC;
		softc.rxInterrupts++;
        rtems_event_send (softc.rxDaemonTid, START_RECEIVE_EVENT);
		DB(printk("|"));
    }

	if ((rsr & (EMAC_RSR_BNA | EMAC_RSR_OVR)) > 0) {         /* Clear all receiver errors if they have occured       */
		softc.rxMissed++;
        EMAC->EMAC_RSR  =  (EMAC_RSR_BNA  | EMAC_RSR_OVR);
		rtems_event_send (softc.rxDaemonTid, START_RECEIVE_EVENT);
		DB(printk(">"));
	}

    if ((status32 & EMAC_ISR_TCOMP) || (tsr & EMAC_TSR_COMP)) {      /* Transmit buffer register empty */
		softc.txInterrupts++;
		...
		EMAC->EMAC_TSR = EMAC_TSR_COL | EMAC_TSR_BEX | EMAC_TSR_UND | EMAC_TSR_RLES | EMAC_TSR_COMP;
		if ((tsr & EMAC_TSR_UBR) > 0)
		{
			EMAC->EMAC_TSR = EMAC_TSR_UBR;
		}
        rtems_event_send (softc.txDaemonTid, START_TRANSMIT_EVENT);
		DB(printk("<"));
    }
