...
volatile sEmacRxDescriptor rxbuf_hdrs[NUM_RXBDS];
volatile sEmacTxDescriptor txbuf_hdrs[NUM_TXBDS];
volatile fEmacdTransferCallback *tx_cbs;

volatile unsigned char  __attribute__ ((aligned(16))) txbuf[NUM_TXBDS * TX_BUFFER_SIZE];
volatile unsigned char  __attribute__ ((aligned(16))) rxbuf[NUM_RXBDS * RX_BUFFER_SIZE];
...
#if 0
static const Pin at91sam9g25_ethernet_led_pins[] = { ETHERNET_LED_PINLIST };
#endif
...
#if 0
	PIO_Configure(at91sam9g25_ethernet_led_pins, PIO_LISTSIZE(at91sam9g25_ethernet_led_pins));
#endif
